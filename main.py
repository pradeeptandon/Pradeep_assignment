...
from networks import Pretrained_Networks as pn
#from lib.sampler import ImbalancedDatasetSampler
...
parser.add_argument('--network', default='SimpleCNN', 
                    choices=['SimpleCNN','resnet','vgg','densenet'],
...
num_classes = train_set.get_num_classes() #7classes are available
...
# Model.
def model_selection(network,num_classes):
    
    if network == 'SimpleCNN':
        net = SimpleCNN(num_classes=num_classes)
    elif network == 'resnet':
        net, input_size = pn.initialize_model(network, num_classes, 
...
net = model_selection(args.network,num_classes)    
net = net.to(device)
if device == 'cuda':
...
if __name__ == '__main__':
    ...
    for epoch in range(0, args.num_epochs):
        ...
        np.save(npy_file, [train_losses, test_losses,train_acc,test_acc])
        create_loss_plot(exp_dir, epochs, train_losses, test_losses)
        create_accuracy_plot(exp_dir, epochs, train_accuracy, test_accuracy)
