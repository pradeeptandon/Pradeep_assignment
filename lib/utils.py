...
def create_accuracy_plot(exp_dir, epochs, train_acc, test_acc):
    """Plot accuracy and save.

    Args:
        exp_dir (str): experiment directory.
        epochs (list): list of epochs (x-axis of loss plot).
        train_accuracy (list): list with train loss during each epoch.
        test_accuracy (list): list with test loss during each epoch.
    ...
    plt.plot(epochs, test_acc, 'r', marker='o', label='test accuracy')
    plt.legend()
    plt.savefig(join(exp_dir,'accuracy.png' ))    
